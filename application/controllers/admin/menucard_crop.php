...
class Menucard_crop extends CI_Controller {
    ...
    public function index() {
        ...
        $data['sImagePath'] = $this->config->item('assets') . "upload/foodmenu/";

        $imageName = $this->session->userdata('uploaded_img');
        $data['sImageName'] = $imageName;
        # PROCESSING ------------------------------------------------------------------
        $this->template->view('image_crop_menucard', $data);
    }
    ...
    public function process($red_to = "") {
        if ($red_to == "closed") {
            ...
            $imageName = $this->session->userdata('uploaded_img');
            $image_path = "assets/upload/foodmenu/" . $imageName;
//            echo $image_path;die;
            $upload_data = array(
                'file_name' => $imageName,
            ...
            if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                if ($_POST['dataW'] > 0 && $_POST['dataH'] > 0) {
                    ...
                    //echo APPPATH;die;
                    $image_path = "assets/upload/foodmenu/" . $_POST['imageName'];
                    ...
                    $image_path = FOODMENU_IMAGE_PATH . $_POST['imageName'];
                    ...
                    $this->session->set_userdata('redirect_to', 'admin/menucard/menucard_list');
                    redirect('admin/menucard/menucard_list');
                }
            }
        }
