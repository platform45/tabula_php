...
class Restaurant_crop extends CI_Controller {
    ...
    public function index() {
        ...
        $data['sImagePath'] = $this->config->item('assets') . "upload/member/";

        $imageName = $this->session->userdata('uploaded_img');
        $data['sImageName'] = $imageName;
//              echo $data['sImagePath'].$data['sImageName'];die;
        # PROCESSING ------------------------------------------------------------------
        $this->template->view('image_crop_restaurant', $data);
    }
    ...
    public function process($red_to = "") {
        if ($red_to == "closed") {
            ...
            $imageName = $this->session->userdata('uploaded_img');
            $image_path = "assets/upload/member/" . $imageName;
//            echo $image_path;die;
            $upload_data = array(
                'file_name' => $imageName,
            ...
            if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                if ($_POST['dataW'] > 0 && $_POST['dataH'] > 0) {
                    ...
                    //echo APPPATH;die;
                    $image_path = "assets/upload/member/" . $_POST['imageName'];
                    ...
                    $image_path = MEMBER_IMAGE_PATH . $_POST['imageName'];
                    ...
                    $this->upload_member_image($upload_data);
    ...
    public function upload_member_image($upload_data = array()) {

        $config_resize['image_library'] = 'gd2';
        $config_resize['source_image'] = $upload_data['full_path'];
        $config_resize['width'] = 800;
        $config_resize['height'] = 560;
    ...
    public function resize_uploaded_if_cancel($upload_data = array()) {
        ...
        $configSize2['source_image'] = MEMBER_IMAGE_PATH . $upload_data['file_name'];
        $configSize2['create_thumb'] = TRUE;
        $configSize2['maintain_ratio'] = TRUE;
        $configSize2['width'] = 800;
        $configSize2['height'] = 560;
        ...
        $configSize1['source_image'] = MEMBER_IMAGE_PATH . $upload_data['file_name'];
        $configSize1['create_thumb'] = FALSE;
        $configSize1['maintain_ratio'] = TRUE;
        $configSize1['width'] = 1600;
        $configSize1['height'] = 1120;
