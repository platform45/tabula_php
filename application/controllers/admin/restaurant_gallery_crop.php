...
class Restaurant_gallery_crop extends CI_Controller {
    ...
    public function index() {
        ...
        $data['sImagePath'] = $this->config->item('assets') . "upload/gallery/";

        $imageName = $this->session->userdata('uploaded_img');
        $data['sImageName'] = $imageName;
        # PROCESSING ------------------------------------------------------------------
        $this->template->view('image_crop_restaurant_gallery', $data);
    }
    ...
    public function process($red_to = "") {
        if ($red_to == "closed") {
            ...
            $image_path = "assets/upload/gallery/" . $imageName;
//            echo $image_path;die;
            $upload_data = array(
                'file_name' => $imageName,
            ...
            if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                if ($_POST['dataW'] > 0 && $_POST['dataH'] > 0) {
                    ...
                    //echo APPPATH;die;
                    $image_path = "assets/upload/gallery/" . $_POST['imageName'];
                    ...
                    $image_path = GALLERY_IMAGE_PATH . $_POST['imageName'];
