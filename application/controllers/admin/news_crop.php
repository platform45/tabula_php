...
/*
 * 	NAME : PK
 *   DESCRIPTION : CNTROLLER FOR CROP NEWS IMGE
 *   DATE : 02-09-2016
 */
?>
<?php

class News_crop extends CI_Controller {
    ...
    public function index() {
        $data['sImagePath'] = $this->config->item('assets') . "upload/news/";
        $imageName = $this->session->userdata('uploaded_img');
        $data['sImageName'] = $imageName;
        # PROCESSING ------------------------------------------------------------------
        $this->template->view('image_crop_news', $data);
    }
    ...
    public function process($red_to = "") {
        if ($red_to == "closed") {
            ...
            $imageName = $this->session->userdata('uploaded_img');
            $image_path = "assets/upload/news/" . $imageName;
            $upload_data = array(
                'file_name' => $imageName,
            ...
            if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                if ($_POST['dataW'] > 0 && $_POST['dataH'] > 0) {
                    ...
                    //echo APPPATH;die;
                    $image_path = "assets/upload/news/" . $_POST['imageName'];
                    ...
                    $image_path = NEWS_IMAGE_PATH . $_POST['imageName'];
                    ...
                    $this->upload_news_image($upload_data);
                    $redirect_to = $this->session->userdata('redirect_to');
                    redirect($redirect_to);
                    die();
                }
    ...
    public function upload_news_image($upload_data = array()) {
    ...
    public function resize_uploaded_if_cancel($upload_data = array()) {
        ...
        $configSize2['source_image'] = NEWS_IMAGE_PATH . $upload_data['file_name'];
        ...
        $configSize1['source_image'] = NEWS_IMAGE_PATH . $upload_data['file_name'];
