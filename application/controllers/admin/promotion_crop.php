...
/*
 * 	NAME : PK
 *   DESCRIPTION : CNTROLLER FOR CROP promotion IMGE
 *   DATE : 02-09-2016
 */
?>
<?php

class Promotion_crop extends CI_Controller {

    public function __construct() {
        parent::__construct();
    }

    //INDEX FUNCTION CALL VIEW
    public function index() {
        $data['sImagePath'] = $this->config->item('assets') . "upload/promotion/";
        $imageName = $this->session->userdata('uploaded_img');
        $data['sImageName'] = $imageName;
        # PROCESSING ------------------------------------------------------------------
        $this->template->view('image_crop_promotion', $data);
    }
    ...
    public function process($red_to = "") {
        if ($red_to == "closed") {
            ...
            $imageName = $this->session->userdata('uploaded_img');
            $image_path = "assets/upload/promotion/" . $imageName;
            $upload_data = array(
                'file_name' => $imageName,
            ...
            if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                if ($_POST['dataW'] > 0 && $_POST['dataH'] > 0) {
                    ...
                    //echo APPPATH;die;
                    $image_path = "assets/upload/promotion/" . $_POST['imageName'];
                    ...
                    $image_path = promotion_IMAGE_PATH . $_POST['imageName'];
                    ...
                    $this->upload_promotion_image($upload_data);
                    $redirect_to = $this->session->userdata('redirect_to');
                    redirect($redirect_to);
                    die();
                }
    ...
    public function upload_promotion_image($upload_data = array()) {
    ...
    public function resize_uploaded_if_cancel($upload_data = array()) {
        ...
        $configSize2['source_image'] = promotion_IMAGE_PATH . $upload_data['file_name'];
        ...
        $configSize1['source_image'] = promotion_IMAGE_PATH . $upload_data['file_name'];
