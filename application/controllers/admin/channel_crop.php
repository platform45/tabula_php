...
class Channel_crop extends CI_Controller
{
    ...
    {
            $data['sImagePath'] = $this->config->item('assets')."upload/brand/";
            $imageName = $this->session->userdata('uploaded_img');
            $data['sImageName'] = $imageName;
            # PROCESSING ------------------------------------------------------------------
            $this->template->view('brand_image_crop',$data);
    }
    ...
    public function process($red_to = ""){
        ...
        if($red_to == "closed"){
            ...
            $imageName = $this->session->userdata('uploaded_img');
            $image_path =  "assets/upload/brand/".$imageName;
            $upload_data = array(
                                    'file_name'=> $imageName,
            ...
            if ($_SERVER['REQUEST_METHOD'] == 'POST') 
			{
                if($_POST['dataW'] > 0 && $_POST['dataH'] > 0) 
                {
                        ...
                        //echo APPPATH;die;
                        $image_path =  "assets/upload/brand/".$_POST['imageName'];
                        ...
                        $image_path = BRAND_IMAGE_PATH .$_POST['imageName'];
   ...
   public function resize_uploaded_if_cancel($upload_data = array())
	{
            ...
            $this->load->library('image_lib');
                ...
                $configSize2['source_image']    = BRAND_IMAGE_PATH.$upload_data['file_name'];
                ...
                $configSize1['source_image']    = BRAND_IMAGE_PATH.$upload_data['file_name'];
