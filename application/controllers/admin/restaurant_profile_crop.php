...
class Restaurant_profile_crop extends CI_Controller {
  ...
  public function index() {
    ...
    $data['sImageName'] = $imageName;
        # PROCESSING ------------------------------------------------------------------
    $this->template->view('image_crop_profile_restaurant', $data);
  }
  ...
  public function process($red_to = "") {
    if ($red_to == "closed") {
      ...
      $this->resize_uploaded_if_cancel($upload_data);
               $edirect_path = $this->session->userdata('redirect_to');
          redirect($edirect_path);
    } else {
