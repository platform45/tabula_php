...
class News_crop extends CI_Controller
{
    ...
    {
            $data['sImagePath'] = $this->config->item('assets')."upload/newsevents/";
            $imageName = $this->session->userdata('uploaded_img');
            $data['sImageName'] = $imageName;
            # PROCESSING ------------------------------------------------------------------
            $this->template->view('news_crop',$data);
    }
    ...
    public function process($red_to = ""){
        if($red_to == "closed"){
            ...
            $imageName = $this->session->userdata('uploaded_img');
            $image_path =  "assets/upload/newsevents/".$imageName;
            $upload_data = array(
                                    'file_name'=> $imageName,
            ...
            if ($_SERVER['REQUEST_METHOD'] == 'POST') 
			{
                if($_POST['dataW'] > 0 && $_POST['dataH'] > 0) 
                {
                        ...
                        //echo APPPATH;die;
                        $image_path =  "assets/upload/newsevents/".$_POST['imageName'];
                        ...
                        $this->upload_newsevents_image($upload_data);
                        $redirect_to = $this->session->userdata('redirect_to');
                        redirect($redirect_to);
                }
            }
    ...
    public function upload_newsevents_image($upload_data = array())
	{
