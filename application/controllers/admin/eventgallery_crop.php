...
class Eventgallery_crop extends CI_Controller
{
    ...
    {
            $data['sImagePath'] = $this->config->item('assets')."upload/eventgallery/";
            $imageName = $this->session->userdata('uploaded_img');
            $data['sImageName'] = $imageName;
            # PROCESSING ------------------------------------------------------------------
            $this->template->view('event_gallery_crop',$data);
    }
    ...
    public function process($red_to = ""){
        ...
        if($red_to == "closed"){
            ...
            $imageName = $this->session->userdata('uploaded_img');
            $image_path =  "assets/upload/eventgallery/".$imageName;
            $upload_data = array(
                                    'file_name'=> $imageName,
            ...
            if ($_SERVER['REQUEST_METHOD'] == 'POST') 
			{
                if($_POST['dataW'] > 0 && $_POST['dataH'] > 0) 
                {
                        ...
                        //echo APPPATH;die;
                        $image_path =  "assets/upload/eventgallery/".$_POST['imageName'];
                        ...
                        $image_path = EVENT_GALLERY_PATH .$_POST['imageName'];
                        ...
                        $this->upload_brand_image($upload_data);
                        $redirect_to = $this->session->userdata('redirect_to');
                        redirect($redirect_to);
                }
            }
   ...
   public function resize_uploaded_if_cancel($upload_data = array())
	{
            ...
            $this->load->library('image_lib');
                ...
                $configSize2['source_image']    = EVENT_GALLERY_PATH.$upload_data['file_name'];
                ...
                $configSize1['source_image']    = EVENT_GALLERY_PATH.$upload_data['file_name'];
