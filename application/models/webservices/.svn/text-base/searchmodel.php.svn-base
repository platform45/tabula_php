...
class SearchModel extends CI_Model {
     ...
     * params:
     *      input: keyword, user_type, latitude, longitude, limit, offset, type (1 for gps /0- search)
     *      output: array
     */

    public function get_search_records($keyword = "", $distance = 0, $latitude, $longitude, $user_type, $limit, $offset,$type) {
        $this->db->select("u.user_id AS restaurant_id,u.latitude,u.longitude,
                        u.user_first_name AS restaurant_name,
        ...
        if($query->num_rows() > 0)
        {
            ...
            $restaurants = $this->restaurantmodel->get_restaurant_cuisine($restaurants);
            return $restaurants;
        }
        else
        {
    ...
    public function get_total_detail_search_records(  $state, $city, $cuisine, $ambience, $min_price, $max_price, $number_of_guest, $date_time, $user_type=3, $dietary_preference)
  {
    $condition_query = $this->_get_query( $cuisine, $ambience, $min_price, $max_price, $number_of_guest, $date_time, $dietary_preference);

    $query1 = "SELECT distinct `u`.`user_id`
              FROM `tab_usermst` u
    ...
    {
      $sql = "SELECT q1.user_id, SUM(q2.capacity) as capacity
            FROM
            (
              ".$query1."
            ) as q1
            ...
            ) as q2 on q1.`user_id` = q2.`user_id`
            GROUP BY q1.user_id
   ...
   *      output: array
   */
  public function get_detail_search_records( $state, $city, $cuisine, $ambience, $min_price, $max_price, $number_of_guest, $date_time, $latitude, $longitude, $user_type=3, $dietary_preference, $limit, $offset )
  {
    $condition_query = $this->_get_query( $cuisine, $ambience, $min_price, $max_price, $number_of_guest, $date_time, $dietary_preference );

    $query1 = "SELECT distinct u.user_id AS restaurant_id,u.latitude,u.longitude,
                u.user_first_name AS restaurant_name,
                u.restaurant_owner_name,
                u.user_email as email,
                IF( u.restaurant_hero_image = '', '', CONCAT('".base_url()."', '".MEMBER_IMAGE_PATH."', u.restaurant_hero_image) ) AS restaurant_image,
  ...
  private function _get_query( $cuisine, $ambience, $min_price, $max_price, $number_of_guest, $date_time,$dietary_preference)
  {
    ...
    if( $min_price >= 0 && $max_price > 0 && $max_price <= SEARCH_MAX_PRICE )
    {
      if( $max_price == SEARCH_MAX_PRICE )
        $price_cond = "AND average_spend >= ".$min_price;
      else
        $price_cond = "AND average_spend BETWEEN ".$min_price." AND ".$max_price;
    }
    ...
    $result_condition = array( 'cuisine_join' => $cuisine_join, 'cuisine_cond' => $cuisine_cond, 'dietary_join' => $dietary_join, 'ambience_join' => $ambience_join, 'ambience_cond' => $ambience_cond, 'price_cond' => $price_cond, 'guest_cond' => $guest_cond,'dietary_cond'=>$dietary_cond , 'query2' => $query2 );
