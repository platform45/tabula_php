...
class Newseventsmodel extends CI_Model
{
    ...
    {
        $this->db->select("`ne_id`, `ne_date`, `ne_title`, `ne_brief`, `ne_image`, `ne_metakeywords`, `ne_metatag`, `ne_desc`, `ne_sequence`, `ne_status`, `is_deleted`",FALSE);
        if($edit_id){
   ...
   {
       $this->db->select("ne_id,ne_title,ne_status",FALSE);
       $this->db->where(
                    array(
                        'is_deleted' => 0
                    ));
    ...
    public function update_status($ne_id = 0)
    {
        ...
        if($query->num_rows() > 0){
            ...
            public function change_sequence($faqid = 0,$change_to = 'up')
            {
                ...
                $result = $this->db->get('aoe_newseventsmst');
                if($result->num_rows() > 0)
                {
                    ...
                    if($other_menu){
                        ...
                        $this->db->where(array('ne_type' => 1));
                        $this->db->update('aoe_newseventsmst',$update_data);
