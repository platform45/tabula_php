...
class Ambiencemodel extends CI_Model {
    ...
    public function getData($edit_id = 0) {
        $this->db->select("ambience_id,ambience_name,is_deleted,status", FALSE);
        if ($edit_id) {
            $this->db->where('ambience_id', $edit_id);
        }
        $this->db->where(
                array(
                    'is_deleted' => '0'
        ));
         $this->db->order_by('ambience_id', 'DESC');
    ...
    public function action($action, $arrData = array(), $edit_id = 0) {
        switch ($action) {
            case 'insert':
                $this->db->insert('ambience', $arrData);
                return $this->db->insert_id();
                break;
            case 'update':
                $this->db->where('ambience_id', $edit_id);
                $this->db->update('ambience', $arrData);
                return $edit_id;
                break;
            case 'delete':
                break;
    ...
    public function update_status($sli_id = 0) {
        ...
        $this->db->where('ambience_id', $sli_id);
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
