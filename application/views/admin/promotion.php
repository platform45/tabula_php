...
<?php $arr = $this->session->userdata('menu'); ?>
        ...
        $("#btn-danger1").click(function(){
            $.ajax({
                url:'<?php echo base_url(); ?>admin/promotion/promotion_count',
                type:"POST",
                success:function(data){
                    if(data>1)
                    {
                        var str = "<?php echo base_url() . 'admin/promotion/delete_promotion/' ?>";
                        var teststr = str;
        ...
        $("#btn-status").click(function(){
            ...
            $.ajax({
                url:'<?php echo base_url(); ?>admin/promotion/promotion_state_count',
                type:"POST",
                success:function(data){
                    if(changeStatus==0)
                    {
        ...
        function change_state()
        {
            ...
            var changeStatus = $("#btn-status").attr("data-status");
            $.ajax({
                url:'<?php echo base_url(); ?>admin/promotion/update_promotion_status',
                type:"POST",
                data:{"promotion_id": $("#btn-status").attr("data-id") },
                success:function(){
                    if(changeStatus == 1)
                    {
...
<div class="content">
    ...
    <div class="main-content">
        ...
        <div id="custom_filter"> 
            <input type="text" class="form-control" style="width:20%;display:inline;" id="filter" name="filter" placeholder="Search Promotion" value="<?php echo set_value('filter'); ?>"/>
            ...
            <a href="<?php echo base_url(); ?>admin/promotion/addedit" class="btn btn-primary pull-right"><i class="fa fa-plus"></i> Add Promotion </a>
        </div>
                    ...
                    if ($sliderData)
                        foreach ($sliderData as $rec):
                            ...
                            <tr style="text-align:center;">
                                <td> <?php echo $i++; ?></td>
                                <td style="text-align:left;"> <?php echo $rec['promotion_title']; ?></td>
                                <td> <img width=100  id="preview" name="preview" src="<?php if ($rec['promotion_image']) echo $this->config->item('assets') . 'upload/promotion/' . $path; else echo $this->config->item('assets') . 'upload/No_Image_thumb.jpg'; ?>"/> </td>
                                                                 <!--  <td style="text-align:left;"> <?php
                    //$originalDate = "2010-03-21";
                    $newDate = date("d-m-Y", strtotime($rec['promotion_date']));
                    echo $newDate;
                            ?></td>-->

                        <!-- <td>
                        <a data-id="<?php echo $rec['promotion_id']; ?>" style="margin-right:15px;" title="Move Up" class="moveup pull-right" id="mp_<?php echo $rec['promotion_id']; ?>" href="<?php echo base_url() . 'admin/slider/change_sequence/up/' . $rec['promotion_id']; ?>">
                        <i class="fa  fa-arrow-circle-up fa-2x"></i>
                        </a>
                        <a data-id="<?php echo $rec['promotion_id']; ?>" style="margin-right:15px;" title="Move Down" class="movedown pull-right" id="mp_<?php echo $rec['promotion_id']; ?>" href="<?php echo base_url() . 'admin/slider/change_sequence/down/' . $rec['promotion_id']; ?>">
                        <i class="fa  fa-arrow-circle-down fa-2x"></i>
                        </a>
                        </td>    -->
                                <td> <a title="Change Status" href="#myStatus" role="button" data-toggle="modal" id="<?php echo $rec['promotion_id']; ?>" class="status" data-status="<?php echo $rec['promotion_status']; ?>"><span id="status<?php echo $rec['promotion_id']; ?>" class="status_icon"><?php if ($rec['promotion_status'] == 0): ?><i class="fa fa-ban fa-2x" ></i><?php else: ?><i class="fa fa-check fa-2x" ></i><?php endif ?></span></a></td>
                                <td><a href="<?php echo base_url() . 'admin/promotion/addedit/' . $rec['promotion_id']; ?>" title="Edit promotion"><i class="fa fa-pencil-square-o fa-2x" ></i></a></td>
                                <td><a href="#myModal" class="delete_button" id="<?php echo $rec['promotion_id']; ?>" role="button" data-toggle="modal" title="Delete"><i class="fa fa-times-circle-o fa-2x" ></i></a></td>
                            </tr>
                        <?php endforeach ?>
                </tbody>
            </table>
