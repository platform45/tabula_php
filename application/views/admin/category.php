...
$(document).ready(function() {
        ...
        */
        $("#btn-danger1").click(function(){
            $.ajax({
                        url:'<?php echo base_url();?>admin/category/category_count',
                        type:"POST",
                        success:function(data){
                                 if(data>1)
                                     {
                                        $.ajax({
                                        url:'<?php echo base_url();?>admin/category/check_having_video',
                                        type:"POST",
                                        data:{category_id : $("#myModal").attr("data-id")},
                                        success:function(data){                                            
                                        if(data==1)
                                            {
                                                var str = "<?php echo base_url().'admin/category/delete_category/'?>";
                                                var teststr = str;
                                            ...
                                            {
                                                $().toastmessage('showErrorToast', "You cannot delete this channel as its includes category");
                                            }
                                            }
                                            });
                                     }
        ...
        function change_state()
        {
            ...
            var spanid= $("#btn-status").attr("data-id");
                var changeStatus = $("#btn-status").attr("data-status");
                $.ajax({
                        url:'<?php echo base_url();?>admin/category/update_category_status',
                        type:"POST",
                        data:{"cat_id": $("#btn-status").attr("data-id") },
                        success:function(){
...
<div class="content">
            ...
            <div class="main-content">
                    ...
                    <div id="custom_filter"> 
                            ...
                            <a href="<?php echo base_url();?>admin/category/addedit" class="btn btn-primary pull-right"><i class="fa fa-plus"></i> Add Category </a>
                    </div>
...
<!--                                <td>
                                        <a data-id="<?php echo $rec['cat_id'];?>" style="margin-right:15px;" title="Move up" class="moveup pull-right" id="mp_<?php echo $rec['cat_id'];?>" href="<?php echo base_url().'admin/channel/change_sequence/up/'.$rec['cat_id'];?>">
                                            <i class="fa  fa-arrow-circle-up fa-2x"></i>
                                        </a>
                                        <a data-id="<?php echo $rec['cat_id'];?>" style="margin-right:15px;" title="Move down" class="movedown pull-right" id="mp_<?php echo $rec['cat_id'];?>" href="<?php echo base_url().'admin/channel/change_sequence/down/'.$rec['cat_id'];?>">
                                            <i class="fa  fa-arrow-circle-down fa-2x"></i>
                                        </a>
                                </td>    -->
                                <td> <a title="Change status" href="#myStatus" role="button" data-toggle="modal" id="<?php echo $rec['cat_id'];?>" class="status" data-status="<?php echo $rec['category_status'];?>"><span id="status<?php echo $rec['cat_id'];?>" class="status_icon"><?php if($rec['category_status']==0):?><i class="fa fa-ban fa-2x" ></i><?php else:?><i class="fa fa-check fa-2x" ></i><?php endif?></span></a></td>
                                <td><a href="<?php echo base_url().'admin/category/addedit/'.$rec['cat_id'];?>" title="Edit category"><i class="fa fa-pencil-square-o fa-2x" ></i></a></td>
                                <td><a href="#myModal" class="delete_button" id="<?php echo $rec['cat_id'];?>" role="button" data-toggle="modal" title="Delete"><i class="fa fa-times-circle-o fa-2x" ></i></a></td>
                            </tr>
                            <?php endforeach ?>
                        </tbody>
                    </table>
