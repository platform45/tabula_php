...
$(document).ready(function() {
        ...
        $(".sidebar-nav #menu<?php echo $arr['Divisions'][1]?>").addClass("act");
        ...
        */
        $("#btn-danger1").click(function(){
            $.ajax({
                        url:'<?php echo base_url();?>divisions/divisions_count',
                        type:"POST",
                        success:function(data){
                                
                                        var str = "<?php echo base_url().'divisions/delete_divisions/'?>";
                                        var teststr = str;
        ...
        $("#btn-status").click(function(){
              var spanid= $("#btn-status").attr("data-id");
                ...
                $.ajax({
                        url:'<?php echo base_url();?>divisions/update_status',
                        type:"POST",
                        data:{"div_id": $("#btn-status").attr("data-id") },
                        success:function(){
       ...
       $("#filter").keypress(function(e){
                ...
                if(keycode==13)
                {
                        if($('#filter').val() != '')
                        {
                                ...
                                oTable
                                        ...
                                        .search( $('#filter').val() )
                                        .draw();
                                $.session.set("divisions_search",test);
        ...
        {
                if($.session.get("divisions_search"))
                {
                        $("#filter").val($.session.get("divisions_search"));
        ...
        $('#clear').click( function ()
        {
                ...
                $.session.remove('divisions_search');
                $('#filter').val('');
...
<div class="content">
            ...
            <div class="main-content">
                    ...
                    <div id="custom_filter"> 
                            <input type="text" class="form-control" style="width:20%;display:inline;" id="filter" name="filter" placeholder="Search divisions" value="<?php echo set_value('filter');?>"/>
                            ...
                            <a href="<?php echo base_url();?>divisions/addedit" class="btn btn-primary pull-right"><i class="fa fa-plus"></i> Create Divisions </a>
                    </div>
				...
				<div class="table-responsive">
                    <table class="display hover cell-border table" id="test">
                        ...
                        <tbody>
                            ...
                            <?php 
                            if($divisionsData)
                            foreach($divisionsData as $rec):
                               ?>
                            <tr style="text-align:center;">
                                <td> <?php echo $i++;?></td>
                                <td style="text-align:left;"> <?php echo $rec['div_title']; ?></td>
                                <td>
                                        <a data-id="<?php echo $rec['div_id'];?>" style="margin-right:15px;" title="Move up" class="moveup pull-right" id="mp_<?php echo $rec['div_id'];?>" href="<?php echo base_url().'divisions/change_sequence/up/'.$rec['div_id'];?>">
                                            <i class="fa  fa-arrow-circle-up fa-2x"></i>
                                        </a>
                                        <a data-id="<?php echo $rec['div_id'];?>" style="margin-right:15px;" title="Move down" class="movedown pull-right" id="mp_<?php echo $rec['div_id'];?>" href="<?php echo base_url().'divisions/change_sequence/down/'.$rec['div_id'];?>">
                                            <i class="fa  fa-arrow-circle-down fa-2x"></i>
                                        </a>
                                </td>    
                                <td> <a href="#myStatus" role="button" data-toggle="modal" id="<?php echo $rec['div_id'];?>" class="status" data-status="<?php echo $rec['div_status'];?>"><span id="status<?php echo $rec['div_id'];?>" class="status_icon"><?php if($rec['div_status']==0):?><i class="fa fa-ban fa-2x" ></i><?php else:?><i class="fa fa-check fa-2x" ></i><?php endif?></span></a></td>
                                <td><a href="<?php echo base_url().'divisions/addedit/'.$rec['div_id'];?>" title="Edit Division"><i class="fa fa-pencil-square-o fa-2x" ></i></a></td>
                                <td><a href="#myModal" class="delete_button" id="<?php echo $rec['div_id'];?>" role="button" data-toggle="modal" title="Delete"><i class="fa fa-times-circle-o fa-2x" ></i></a></td>
                            </tr>
                            <?php endforeach ?>
                        </tbody>
                    </table>
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                            <p class="error-text"><i class="fa fa-warning modal-icon"></i>Are you sure you want to delete this divisions?<br>This cannot be undone.</p>
                                    </div>
