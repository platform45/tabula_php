...
<?php $arr = $this->session->userdata('menu') ?>
                ...
                $("#btn-danger1").click(function(){
                    $.ajax({
                        url:'<?php echo base_url(); ?>admin/event/event_count',
                        type:"POST",
                        success:function(data){
                                
                            var str = "<?php echo base_url() . 'admin/event/delete_event/' ?>";
                            var teststr = str;
                ...
                $("#btn-status").click(function(){
                    ...
                    $.ajax({
                        url:'<?php echo base_url(); ?>admin/event/update_status',
                        type:"POST",
                        data:{"ne_id": $("#btn-status").attr("data-id") },
                        success:function(){
                ...
                if($.session.get("addedit")==1)
                {
                    ...
                    {
                        $("#filter").val($.session.get("event_search"));
...
<div class="content">
    ...
    <div class="main-content">
        ...
        <div id="custom_filter"> 
            <input type="text" class="form-control" style="width:20%;display:inline;" id="filter" name="filter" placeholder="Search Event" value="<?php echo set_value('filter'); ?>"/>
            ...
            <a href="<?php echo base_url(); ?>admin/event/addedit" class="btn btn-primary pull-right"><i class="fa fa-plus"></i> Add Event </a>
        </div>
            ...
            <table class="display hover cell-border table" id="test">
                <thead>
                    <tr class="table_th_tr">
                        ...
                        <th  class="row-col_1">ADD IMAGES</th>
                ...
                <tbody>
                    ...
                    <?php
                    if ($eventData)
                        foreach ($eventData as $rec):
                            ?>
                            <tr style="text-align:center;">
                                <td> <?php echo $i++; ?></td>
                                <td style="text-align:left;"> <?php echo $rec['ne_title']; ?></td>
                                <td><?php echo date('m-d-Y', strtotime($rec['ne_date'])); ?></td>
                                <td><?php echo date('m-d-Y', strtotime($rec['ne_to_date'])); ?></td>

                                <td><a href="<?php echo base_url() ?>admin/eventgallery/index/<?php echo $rec['ne_id']; ?>" title="Add Images"><i  class="fa fa-plus fa-2x" ></i></a></td>                               
                                <td>
                                    <a data-id="<?php echo $rec['ne_id']; ?>" style="margin-right:15px;" title="Move up" class="moveup pull-right" id="mp_<?php echo $rec['ne_id']; ?>" href="<?php echo base_url() . 'admin/event/change_sequence/up/' . $rec['ne_id']; ?>">
                                        <i class="fa  fa-arrow-circle-up fa-2x"></i>
                                    </a>
                                    <a data-id="<?php echo $rec['ne_id']; ?>" style="margin-right:15px;" title="Move down" class="movedown pull-right" id="mp_<?php echo $rec['ne_id']; ?>" href="<?php echo base_url() . 'admin/event/change_sequence/down/' . $rec['ne_id']; ?>">
                                        <i class="fa  fa-arrow-circle-down fa-2x"></i>
                                    </a>
                                </td> 

                                <td> <a href="#myStatus" role="button" data-toggle="modal" id="<?php echo $rec['ne_id']; ?>" class="status" data-status="<?php echo $rec['ne_status']; ?>"><span id="status<?php echo $rec['ne_id']; ?>" class="status_icon"><?php if ($rec['ne_status'] == 0): ?><i class="fa fa-ban fa-2x" ></i><?php else: ?><i class="fa fa-check fa-2x" ></i><?php endif ?></span></a></td>
                                <td><a href="<?php echo base_url() . 'admin/event/addedit/' . $rec['ne_id']; ?>" title="Edit Event"><i class="fa fa-pencil-square-o fa-2x" ></i></a></td>
                                <td><a href="#myModal" class="delete_button" id="<?php echo $rec['ne_id']; ?>" role="button" data-toggle="modal" title="Delete"><i class="fa fa-times-circle-o fa-2x" ></i></a></td>
                            </tr>
                        <?php endforeach ?>
                </tbody>
            </table>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <p class="error-text"><i class="fa fa-warning modal-icon"></i>Are you sure you want to delete this Event ?<br>This cannot be undone.</p>
                    </div>
