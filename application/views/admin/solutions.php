...
$(document).ready(function() {
        ...
        */
        $("#btn-danger1").click(function(){
            $.ajax({
                        url:'<?php echo base_url();?>solutions/solutions_count',
                        type:"POST",
                        success:function(data){
                                        var str = "<?php echo base_url().'solutions/delete_solutions/'?>";
                                        var teststr = str;
        ...
        $("#btn-status").click(function(){
              var spanid= $("#btn-status").attr("data-id");
                ...
                $.ajax({
                        url:'<?php echo base_url();?>solutions/update_status',
                        type:"POST",
                        data:{"sol_id": $("#btn-status").attr("data-id") },
                        success:function(){
       ...
       $("#filter").keypress(function(e){
                ...
                if(keycode==13)
                {
                        if($('#filter').val() != '')
                        {
                                ...
                                oTable
                                        ...
                                        .search( $('#filter').val() )
                                        .draw();
                                $.session.set("solutions_search",test);
        ...
        {
                if($.session.get("solutions_search"))
                {
                        $("#filter").val($.session.get("solutions_search"));
        ...
        $('#clear').click( function ()
        {
                ...
                $.session.remove('solutions_search');
                $('#filter').val('');
...
<div class="content">
            ...
            <div class="main-content">
                    ...
                    <div id="custom_filter"> 
                            <input type="text" class="form-control" style="width:20%;display:inline;" id="filter" name="filter" placeholder="Search solutions" value="<?php echo set_value('filter');?>"/>
                            ...
                            <a href="<?php echo base_url();?>solutions/addedit" class="btn btn-primary pull-right"><i class="fa fa-plus"></i> Create Solutions </a>
                    </div>
				...
				<div class="table-responsive">
                    <table class="display hover cell-border table" id="test">
                        ...
                        <tbody>
                            ...
                            <?php 
                            if($solutionsData)
                            foreach($solutionsData as $rec):
                               ?>
                            <tr style="text-align:center;">
                                <td> <?php echo $i++;?></td>
                                <td style="text-align:left;"> <?php echo $rec['sol_title']; ?></td>
                                <td>
                                        <a data-id="<?php echo $rec['sol_id'];?>" style="margin-right:15px;" title="Move up" class="moveup pull-right" id="mp_<?php echo $rec['sol_id'];?>" href="<?php echo base_url().'solutions/change_sequence/up/'.$rec['sol_id'];?>">
                                            <i class="fa  fa-arrow-circle-up fa-2x"></i>
                                        </a>
                                        <a data-id="<?php echo $rec['sol_id'];?>" style="margin-right:15px;" title="Move down" class="movedown pull-right" id="mp_<?php echo $rec['sol_id'];?>" href="<?php echo base_url().'solutions/change_sequence/down/'.$rec['sol_id'];?>">
                                            <i class="fa  fa-arrow-circle-down fa-2x"></i>
                                        </a>
                                </td>    
                                <td> <a href="#myStatus" role="button" data-toggle="modal" id="<?php echo $rec['sol_id'];?>" class="status" data-status="<?php echo $rec['sol_status'];?>"><span id="status<?php echo $rec['sol_id'];?>" class="status_icon"><?php if($rec['sol_status']==0):?><i class="fa fa-ban fa-2x" ></i><?php else:?><i class="fa fa-check fa-2x" ></i><?php endif?></span></a></td>
                                <td><a href="<?php echo base_url().'solutions/addedit/'.$rec['sol_id'];?>" title="Edit Solution"><i class="fa fa-pencil-square-o fa-2x" ></i></a></td>
                                <td><a href="#myModal" class="delete_button" id="<?php echo $rec['sol_id'];?>" role="button" data-toggle="modal" title="Delete"><i class="fa fa-times-circle-o fa-2x" ></i></a></td>
                            </tr>
                            <?php endforeach ?>
                        </tbody>
                    </table>
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                            <p class="error-text"><i class="fa fa-warning modal-icon"></i>Are you sure you want to delete this solutions?<br>This cannot be undone.</p>
                                    </div>
