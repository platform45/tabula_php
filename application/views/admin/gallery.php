...
<?php $arr = $this->session->userdata('menu'); ?>
        ...
        $("#btn-danger1").click(function(){
            $.ajax({
                url:'<?php echo base_url(); ?>admin/gallery/slider_count',
                type:"POST",
                success:function(data){
                    if(data>1)
                    {
                        var str = "<?php echo base_url() . 'admin/gallery/delete_slider/' ?>";
                        var teststr = str;
        ...
        $("#btn-status").click(function(){
            ...
            $.ajax({
                url:'<?php echo base_url(); ?>admin/gallery/slider_state_count',
                type:"POST",
                success:function(data){
                    if(changeStatus==0)
                    {
        ...
        function change_state()
        {
            ...
            var changeStatus = $("#btn-status").attr("data-status");
            $.ajax({
                url:'<?php echo base_url(); ?>admin/gallery/update_slider_status',
                type:"POST",
...
<div class="content">
    ...
    <div class="main-content">
        ...
        <div id="custom_filter"> 
            <input type="text" class="form-control" style="width:20%;display:inline;" id="filter" name="filter" placeholder="Search gallery" value="<?php echo set_value('filter'); ?>"/>
            ...
            <a href="<?php echo base_url(); ?>admin/gallery/addedit" class="btn btn-primary pull-right"><i class="fa fa-plus"></i> Add Gallery </a>
        </div>
        <br/>
                    ...
                    if ($sliderData)
                        foreach ($sliderData as $rec):
                            ...
                            <tr style="text-align:center;">
                                <td> <?php echo $i++; ?></td>
                                <td style="text-align:left;"> <?php echo $rec['sli_title']; ?></td>
                                <td> <img width=100  id="preview" name="preview" src="<?php if ($rec['sli_image']) echo $this->config->item('assets') . 'upload/slider/' . $thumb_img; else echo $this->config->item('assets') . 'upload/No_Image_thumb.jpg'; ?>"/> </td>
                                <td>
                                    <a data-id="<?php echo $rec['sli_id']; ?>" style="margin-right:15px;" title="Move up" class="moveup pull-right" id="mp_<?php echo $rec['sli_id']; ?>" href="<?php echo base_url() . 'admin/gallery/change_sequence/up/' . $rec['sli_id']; ?>">
                                        <i class="fa  fa-arrow-circle-up fa-2x"></i>
                                    </a>
                                    <a data-id="<?php echo $rec['sli_id']; ?>" style="margin-right:15px;" title="Move down" class="movedown pull-right" id="mp_<?php echo $rec['sli_id']; ?>" href="<?php echo base_url() . 'admin/gallery/change_sequence/down/' . $rec['sli_id']; ?>">
                                ...
                                <td><a href="<?php echo base_url() . 'admin/gallery/addedit/' . $rec['sli_id']; ?>" title="Edit gallery"><i class="fa fa-pencil-square-o fa-2x" ></i></a></td>
