<script type="text/javascript" >
    $(document).ready(function() {
        ...
        $("#btn-danger1").click(function(){
            $.ajax({
                url:'<?php echo base_url() . "admin/restaurant_gallery/slider_count/" ?>',
                type:"POST",
                success:function(data){
                    if(data>1)
                    {
                        var str = "<?php echo base_url() . 'admin/restaurant_gallery/delete_slider/' ?>";
                        var teststr = str;
        ...
        $("#btn-status").click(function(){
            ...
            $.ajax({
                url:'<?php echo base_url() . "admin/restaurant_gallery/slider_state_count/" ?>',
                type:"POST",
                success:function(data){
                    if(changeStatus==0)
                    {
        ...
        function change_state()
        {
            ...
            var changeStatus = $("#btn-status").attr("data-status");
            $.ajax({
                url:'<?php echo base_url() . "admin/restaurant_gallery/update_slider_status/" ?>',
                type:"POST",
                data:{"gal_id": $("#btn-status").attr("data-id") },
                success:function(){
                    if(changeStatus == 1)
                    {
...
<div class="content">
    ...
    <div class="main-content">
        ...
        <div id="custom_filter">
            ...
            <a href="<?php echo base_url() . 'admin/restaurant_gallery/addedit/' ?>" class="btn btn-primary pull-right"><i class="fa fa-plus"></i> Add Gallery </a><br><br>
        </div>
        <br/>
                ...
                if ($sliderData)
                    foreach ($sliderData as $rec):
                        ...
                        <tr style="text-align:center; ">
                            <td> <?php echo $i++; ?></td>
                            <td> <img height="100px" width="auto"  id="preview" name="preview" src="<?php if ($rec['gal_image']) echo $this->config->item('assets') . 'upload/gallery/' . $thumb_img; else echo $this->config->item('assets') . 'upload/No_Image_thumb.jpg'; ?>"/> </td>
                            <td>
                                <a data-id="<?php echo $rec['gal_id']; ?>" style="margin-right:15px;" title="Move up" class="moveup pull-right" id="mp_<?php echo $rec['gal_id']; ?>" href="<?php echo base_url() . 'admin/restaurant_gallery/change_sequence/up/' . $user_id . '/' . $rec['gal_id']; ?>">
                                    <i class="fa  fa-arrow-circle-up fa-2x"></i>
                                </a>
                                <a data-id="<?php echo $rec['gal_id']; ?>" style="margin-right:15px;" title="Move down" class="movedown pull-right" id="mp_<?php echo $rec['gal_id']; ?>" href="<?php echo base_url() . 'admin/restaurant_gallery/change_sequence/down/' . $user_id . '/' . $rec['gal_id']; ?>">
                                    <i class="fa  fa-arrow-circle-down fa-2x"></i>
                                </a>
                            </td>
                            <td> <a title="Change status" href="#myStatus" role="button" data-toggle="modal" id="<?php echo $rec['gal_id']; ?>" class="status" data-status="<?php echo $rec['gal_status']; ?>"><span id="status<?php echo $rec['gal_id']; ?>" class="status_icon"><?php if ($rec['gal_status'] == 0): ?><i class="fa fa-ban fa-2x" ></i><?php else: ?><i class="fa fa-check fa-2x" ></i><?php endif ?></span></a></td>
                            <td><a href="#myModal" class="delete_button" id="<?php echo $rec['gal_id']; ?>" role="button" data-toggle="modal" title="Delete"><i class="fa fa-times-circle-o fa-2x" ></i></a></td>
                        </tr>
                    <?php endforeach ?>
                </tbody>
            </table>
