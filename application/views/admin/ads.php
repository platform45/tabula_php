...
<?php $arr = $this->session->userdata('menu') ?>
        ...
        $("#btn-danger1").click(function(){
            $.ajax({
                url:'<?php echo base_url(); ?>admin/ads/ad_count',
                type:"POST",
                success:function(data){
                    if(data>1)
                    {
                        var str = "<?php echo base_url() . 'admin/ads/delete_ad/' ?>";
                        var teststr = str;
        ...
        function change_state()
        {
            ...
            var changeStatus = $("#btn-status").attr("data-status");
            $.ajax({
                url:'<?php echo base_url(); ?>admin/ads/update_ad_status',
                type:"POST",
                data:{"ad_id": $("#btn-status").attr("data-id") },
                success:function(data){
                    //alert(data);
                    if(changeStatus == 1)
                    {
        ...
        {
            if($.session.get("ads_search"))
            {
                $("#filter").val($.session.get("ads_search"));
...
<div class="content">
    <input type="hidden" name="fp" id="fp" value="<?php
if (isset($fp)) {
    echo $fp;
}
?>" />
    ...
    <div class="main-content">
        ...
        <div id="custom_filter"> 
            <input type="text" class="form-control" style="width:20%;display:inline;" id="filter" name="filter" placeholder="Search ad" value="<?php echo set_value('filter'); ?>"/>
            ...
            <a style="margin-left: 10px;" href="<?php echo base_url(); ?>admin/ads/addedit" class="btn btn-primary pull-right add_button"><i class="fa fa-plus"></i> Add Ad </a>
            ...
            <table class="display hover cell-border table" id="test">
                <thead>
                    <tr class="table_th_tr">
                        ...
                        <th class="row-col_3 " style="text-align:center;">AD NAME</th>
                ...
                <tbody>
                    ...
                    <?php
                    if ($adsData)
                        foreach ($adsData as $rec):
                            ?>
                            <tr style="text-align:center;">
                                <td> <?php echo $i++; ?></td>
                                <td style="text-align:left;"> <?php echo $rec['ad_title']; ?></td>
                                <td style="text-align:left;"> <?php echo $rec['channel_title']; ?></td>           


                                <td id="<?php echo $rec['ad_id']; ?>" class="status"> <a href="#myStatus" role="button" data-toggle="modal" id="<?php echo $rec['ad_id']; ?>" class="status" data-status="<?php echo $rec['ad_status']; ?>"><span id="status<?php echo $rec['ad_id']; ?>" class="status_icon"><?php if ($rec['ad_status'] == 0): ?><i class="fa fa-ban fa-2x" ></i><?php else: ?><i class="fa fa-check fa-2x" ></i><?php endif ?></span></a></td>

                                <td class="edit"><a href="<?php echo base_url() . 'admin/ads/addedit/' . $rec['ad_id']; ?>" title="Edit ads"><i class="fa fa-pencil-square-o fa-2x" ></i></a></td>
                                <td class="delete"><a href="#myModal" class="delete_button" id="<?php echo $rec['ad_id']; ?>" role="button" data-toggle="modal" title="Delete"><i class="fa fa-times-circle-o fa-2x" ></i></a></td>
                            </tr>
                        <?php endforeach ?>
                </tbody>
            </table>
        ...
        <div class="modal small fade" id="myhomeStatus" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">X</button>
                    ...
                    <div class="modal-body">
                        <p class="error-text"><i class="fa fa-warning modal-icon"></i>Are you sure you want to change the status for display at home?</p>
                    </div>
                    <div class="modal-footer">
                        <button class="btn btn-default" data-dismiss="modal" aria-hidden="true">Cancel</button>
                        <button class="btn btn-primary" id="btn-homestatus" data-dismiss="modal">Change</button>
                    </div>
                </div>
            </div>
        </div>
