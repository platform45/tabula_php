...
$(document).ready(function() {
        ...
        $(".sidebar-nav #menu<?php echo $arr['Industries'][1]?>").addClass("act");
        ...
        */
        $("#btn-danger1").click(function(){
            $.ajax({
                        url:'<?php echo base_url();?>industries/industries_count',
                        type:"POST",
                        success:function(data){
                                
                                        var str = "<?php echo base_url().'industries/delete_industries/'?>";
                                        var teststr = str;
        ...
        $("#btn-status").click(function(){
              var spanid= $("#btn-status").attr("data-id");
                ...
                $.ajax({
                        url:'<?php echo base_url();?>industries/update_status',
                        type:"POST",
                        data:{"ind_id": $("#btn-status").attr("data-id") },
                        success:function(){
       ...
       $("#filter").keypress(function(e){
                ...
                if(keycode==13)
                {
                        if($('#filter').val() != '')
                        {
                                ...
                                oTable
                                        ...
                                        .search( $('#filter').val() )
                                        .draw();
                                $.session.set("industries_search",test);
        ...
        {
                if($.session.get("industries_search"))
                {
                        $("#filter").val($.session.get("industries_search"));
        ...
        $('#clear').click( function ()
        {
                ...
                $.session.remove('industries_search');
                $('#filter').val('');
...
<div class="content">
            ...
            <div class="main-content">
                    ...
                    <div id="custom_filter"> 
                            <input type="text" class="form-control" style="width:20%;display:inline;" id="filter" name="filter" placeholder="Search industries" value="<?php echo set_value('filter');?>"/>
                            ...
                            <a href="<?php echo base_url();?>industries/addedit" class="btn btn-primary pull-right"><i class="fa fa-plus"></i> Create Industries </a>
                    </div>
				...
				<div class="table-responsive">
                    <table class="display hover cell-border table" id="test">
                        ...
                        <tbody>
                            ...
                            <?php 
                            if($industriesData)
                            foreach($industriesData as $rec):
                               ?>
                            <tr style="text-align:center;">
                                <td> <?php echo $i++;?></td>
                                <td style="text-align:left;"> <?php echo $rec['ind_title']; ?></td>
                                <td>
                                        <a data-id="<?php echo $rec['ind_id'];?>" style="margin-right:15px;" title="Move up" class="moveup pull-right" id="mp_<?php echo $rec['ind_id'];?>" href="<?php echo base_url().'industries/change_sequence/up/'.$rec['ind_id'];?>">
                                            <i class="fa  fa-arrow-circle-up fa-2x"></i>
                                        </a>
                                        <a data-id="<?php echo $rec['ind_id'];?>" style="margin-right:15px;" title="Move down" class="movedown pull-right" id="mp_<?php echo $rec['ind_id'];?>" href="<?php echo base_url().'industries/change_sequence/down/'.$rec['ind_id'];?>">
                                            <i class="fa  fa-arrow-circle-down fa-2x"></i>
                                        </a>
                                </td>    
                                <td> <a href="#myStatus" role="button" data-toggle="modal" id="<?php echo $rec['ind_id'];?>" class="status" data-status="<?php echo $rec['ind_status'];?>"><span id="status<?php echo $rec['ind_id'];?>" class="status_icon"><?php if($rec['ind_status']==0):?><i class="fa fa-ban fa-2x" ></i><?php else:?><i class="fa fa-check fa-2x" ></i><?php endif?></span></a></td>
                                <td><a href="<?php echo base_url().'industries/addedit/'.$rec['ind_id'];?>" title="Edit Industry"><i class="fa fa-pencil-square-o fa-2x" ></i></a></td>
                                <td><a href="#myModal" class="delete_button" id="<?php echo $rec['ind_id'];?>" role="button" data-toggle="modal" title="Delete"><i class="fa fa-times-circle-o fa-2x" ></i></a></td>
                            </tr>
                            <?php endforeach ?>
                        </tbody>
                    </table>
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                            <p class="error-text"><i class="fa fa-warning modal-icon"></i>Are you sure you want to delete this industries?<br>This cannot be undone.</p>
                                    </div>
