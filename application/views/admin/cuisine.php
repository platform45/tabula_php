...
<?php $arr = $this->session->userdata('menu'); ?>
        ...
        $("#btn-danger1").click(function(){
            $.ajax({
                url:'<?php echo base_url(); ?>admin/cuisine/slider_count',
                type:"POST",
                success:function(data){
                    if(data>1)
                    {
                        var str = "<?php echo base_url() . 'admin/cuisine/delete_slider/' ?>";
                        var teststr = str;
        ...
        $("#btn-status").click(function(){
            ...
            $.ajax({
                url:'<?php echo base_url(); ?>admin/cuisine/slider_state_count',
                type:"POST",
                success:function(data){
                    if(changeStatus==0)
                    {
        ...
        function change_state()
        {
            ...
            var changeStatus = $("#btn-status").attr("data-status");
            $.ajax({
                url:'<?php echo base_url(); ?>admin/cuisine/update_slider_status',
                type:"POST",
                data:{"cuisine_id": $("#btn-status").attr("data-id") },
                success:function(){
                    if(changeStatus == 1)
                    {
...
<div class="content">
    ...
    <div class="main-content">
        ...
        <div id="custom_filter"> 
            <input type="text" class="form-control" style="width:20%;display:inline;" id="filter" name="filter" placeholder="Search cuisine" value="<?php echo set_value('filter'); ?>"/>
            ...
            <a href="<?php echo base_url(); ?>admin/cuisine/addedit" class="btn btn-primary pull-right"><i class="fa fa-plus"></i> Add Cuisine </a>
        </div>
        <br/>
        <div class="table-responsive">
            <table class="display hover cell-border table" id="test">
                ...
                <tbody>
                    ...
                    <?php
                    if ($cuisineData)
                        foreach ($cuisineData as $rec):
                            ?>
                            <tr style="text-align:center;">
                                <td> <?php echo $i++; ?></td>
                                <td style="text-align:center;"> <?php echo $rec['cuisine_name']; ?></td>

                                <td> <a title="Change status" href="#myStatus" role="button" data-toggle="modal" id="<?php echo $rec['cuisine_id']; ?>" class="status" data-status="<?php echo $rec['status']; ?>"><span id="status<?php echo $rec['cuisine_id']; ?>" class="status_icon"><?php if ($rec['status'] == 0): ?><i class="fa fa-ban fa-2x" ></i><?php else: ?><i class="fa fa-check fa-2x" ></i><?php endif ?></span></a></td>
                                <td><a href="<?php echo base_url() . 'admin/cuisine/addedit/' . $rec['cuisine_id']; ?>" title="Edit Cuisine"><i class="fa fa-pencil-square-o fa-2x" ></i></a></td>
                                <td><a href="#myModal" class="delete_button" id="<?php echo $rec['cuisine_id']; ?>" role="button" data-toggle="modal" title="Delete"><i class="fa fa-times-circle-o fa-2x" ></i></a></td>
                            </tr>
                        <?php endforeach ?>
                </tbody>
            </table>
